...
void VueOpenGL::dessineAccelerateur(Accelerateur const * acc,QMatrix4x4 const& point_de_vue){
   ...
   for(auto f:acc->getFaisceaux()){
       for(auto p:f->getParticules()){
           ...
           c.R = f->get_couleur().get_x() + electric_charge*0.01;
           c.G = f->get_couleur().get_y() + electric_charge*0.01;
           c.B = f->get_couleur().get_z() + electric_charge*0.01;
...
void VueOpenGL::initializePosition()
{
  ...
  //matrice_vue.rotate(90, 0.0, 1.0, 0.0);
